...
当前功能：PA18 启动 A→B 任务，到达B附近后通过灰度偏离中心识别圆弧并停车；PB22 启动一圈循迹任务，灰度PID持续通过B/C/D两侧圆弧，回到A附近后识别5cm终点横线，再继续循迹约130～180mm，使终点线进入车身下方后减速制动停车；PB23在任何阶段都可立即停车。两种任务都使用缓慢升速、编码器里程保护、短时反向制动和LCD计时。LCD同时显示陀螺仪Z轴角速度、任务状态和距离。
