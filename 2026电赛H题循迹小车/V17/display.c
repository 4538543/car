...
static char gAngleCurrent[ANGLE_FIELD_LENGTH + 1U];
static char gAngleDesired[ANGLE_FIELD_LENGTH + 1U];
...
static void formatAngle(void)
{
    int32_t angle = Gyro_getIntegratedAngleCentiDeg();
    uint32_t magnitude;

    if (angle < 0) {
        gAngleDesired[0] = '-';
        magnitude = (uint32_t) (-(int64_t) angle);
    } else {
        gAngleDesired[0] = '+';
    ...
    writeUnsignedFixed(&gAngleDesired[1], magnitude / 100U, 4U);
    gAngleDesired[5] = '.';
    writeUnsignedFixed(&gAngleDesired[6], magnitude % 100U, 2U);
    gAngleDesired[8] = 'D';
    gAngleDesired[9] = 'G';
    gAngleDesired[10] = '\0';
}
...
void Display_init(void)
{
    ...
    Lcd_drawText(10U, 44U, "WZ:", 2U,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    Lcd_drawText(10U, 76U, "ANGLE:", 2U,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    Lcd_drawText(10U, 108U, "TIME:", 2U,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    Lcd_drawText(10U, 140U, "STATE:", 2U,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    Lcd_drawText(10U, 172U, "DIST:", 2U,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    Lcd_drawText(10U, 204U, "SPEED:", 2U,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);

    copyFixed(gWzCurrent, "NO DATA     ", WZ_FIELD_LENGTH);
    copyFixed(gWzDesired, "NO DATA     ", WZ_FIELD_LENGTH);
    copyFixed(gAngleCurrent, "+0000.00DG", ANGLE_FIELD_LENGTH);
    copyFixed(gAngleDesired, "+0000.00DG", ANGLE_FIELD_LENGTH);
    ...
    Lcd_drawText(110U, 44U, gWzCurrent, DISPLAY_TEXT_SCALE,
        LCD_COLOR_YELLOW, LCD_COLOR_BLACK);
    Lcd_drawText(110U, 76U, gAngleCurrent, DISPLAY_TEXT_SCALE,
        LCD_COLOR_MAGENTA, LCD_COLOR_BLACK);
    Lcd_drawText(110U, 108U, gTimeCurrent, DISPLAY_TEXT_SCALE,
        LCD_COLOR_WHITE, LCD_COLOR_BLACK);
    Lcd_drawText(110U, 140U, gStateCurrent, DISPLAY_TEXT_SCALE,
        LCD_COLOR_GREEN, LCD_COLOR_BLACK);
    Lcd_drawText(110U, 172U, gDistanceCurrent, DISPLAY_TEXT_SCALE,
        LCD_COLOR_WHITE, LCD_COLOR_BLACK);
    Lcd_drawText(110U, 204U, gSpeedCurrent, DISPLAY_TEXT_SCALE,
        LCD_COLOR_CYAN, LCD_COLOR_BLACK);
    ...
    gFields[1].x = 110U;
    gFields[1].y = 76U;
    gFields[1].length = ANGLE_FIELD_LENGTH;
    gFields[1].current = gAngleCurrent;
    gFields[1].desired = gAngleDesired;
    gFields[1].foreground = LCD_COLOR_MAGENTA;

    gFields[2].x = 110U;
    gFields[2].y = 108U;
    gFields[2].length = TIME_FIELD_LENGTH;
    gFields[2].current = gTimeCurrent;
    gFields[2].desired = gTimeDesired;
    gFields[2].foreground = LCD_COLOR_WHITE;

    gFields[3].x = 110U;
    gFields[3].y = 140U;
    gFields[3].length = STATE_FIELD_LENGTH;
    gFields[3].current = gStateCurrent;
    gFields[3].desired = gStateDesired;
    gFields[3].foreground = LCD_COLOR_GREEN;

    gFields[4].x = 110U;
    gFields[4].y = 172U;
    gFields[4].length = DIST_FIELD_LENGTH;
    gFields[4].current = gDistanceCurrent;
    gFields[4].desired = gDistanceDesired;
    gFields[4].foreground = LCD_COLOR_WHITE;

    gFields[5].x = 110U;
    gFields[5].y = 204U;
    gFields[5].length = SPEED_FIELD_LENGTH;
    gFields[5].current = gSpeedCurrent;
    gFields[5].desired = gSpeedDesired;
    gFields[5].foreground = LCD_COLOR_CYAN;
...
void Display_task1ms(void)
{
     ...
     */
    for (attempts = 0U; attempts < 6U; attempts++) {
        DisplayField *field = &gFields[gNextField];
        gNextField = (uint8_t) ((gNextField + 1U) % 6U);
