...
static void serviceLapMode(uint32_t averageCount)
{
    ...
    if ((gMode == AB_MISSION_MODE_LAP_PASS) &&
        (gState == AB_MISSION_CRUISE) &&
    ...
    if ((gMode == AB_MISSION_MODE_LAP_IMMEDIATE) &&
        ...
        (GraySensor_getActiveCount() == 3U) &&
        (Gyro_getIntegratedAngleCentiDeg() >=
            LAP_DISTANCE_STOP_MIN_ANGLE_CENTIDEG)) {
        AppTime_stop();
        beginControlledStop();
        return;
    }
    ...
    if ((gMode == AB_MISSION_MODE_LAP_PASS) &&
        (lapCount >= LAP_MISSED_MARKER_RELATIVE_COUNT)) {
        finishMission(AB_MISSION_FAULT);
        return;
    }
