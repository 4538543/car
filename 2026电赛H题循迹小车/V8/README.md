...
三种任务的首次上车步骤见`直线测试说明.md`。起步后第一次启停横线会被记录为一圈距离基准，不会触发完成；第二次横线相对第一次横线判断，因此探头放在启停线后方的距离不会混入一圈长度。正常横线只要任意3～8个探头见黑即可；如果终点位于圆弧出口、车头尚未完全转正，相对行程约5.7m后也接受最左或最右两个相邻探头的终点特征。两类终点都要求此前1秒内见过普通循迹线。圆弧控制在连续检测到外侧误差后保留约36‰持续差速，并把弯道基础PWM限制为165‰，避免“沿切线直走—突然纠偏”形成多边形轨迹。PA18/PB22/PB23都由灰度PID负责方向，编码器用于里程、速度、350mm车长补偿和堵转检测。
