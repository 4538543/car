    ...
    int32_t rampDownStep, int32_t curveBasePermille)
{
    ...
    uint8_t rampDownIntervalTicks = 1U;
    ...
    uint8_t segmentRampIntervalTicks = 1U;
    ...
    if ((mode == AB_MISSION_MODE_AB_PASS) ||
        ...
        rampDownIntervalTicks =
            PB22_PB23_RAMP_DOWN_INTERVAL_10MS_TICKS;
        segmentRampIntervalTicks =
            PB22_PB23_SEGMENT_RAMP_INTERVAL_10MS_TICKS;
        if (mode == AB_MISSION_MODE_AB_PASS) {
            rampUpIntervalTicks =
...
static void serviceLapMode(uint32_t averageCount)
{
    ...
    /*
     * PA18 is deliberately distance-only. Do not capture or evaluate either
     * the launch stripe or the finish stripe; brake at 5810 mm regardless of
     * the grayscale pattern seen there.
     */
    if (gMode == AB_MISSION_MODE_LAP_IMMEDIATE) {
        if (distanceMm >= PA18_FORCE_STOP_DISTANCE_MM) {
            AppTime_stop();
            beginBrakeWait();
        }
        return;
