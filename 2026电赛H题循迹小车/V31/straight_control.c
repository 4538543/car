...
static uint8_t gRampUpIntervalTicks;
static uint8_t gRampUpIntervalCounter;
static uint8_t gRampDownIntervalTicks;
static uint8_t gRampDownIntervalCounter;
static uint32_t gStartupFullSpeedDistanceMm;
...
static int32_t gSegmentRampUpStep;
static int32_t gSegmentRampDownStep;
static uint8_t gSegmentRampIntervalTicks;
static uint8_t gSegmentRampIntervalCounter;
...
static void updateRamp(uint32_t distanceMm)
{
    int32_t effectiveTarget = gTargetBasePermille;

    if ((effectiveTarget > PB22_PB23_STARTUP_MIN_PWM_PERMILLE) &&
        (gStartupFullSpeedDistanceMm > 0U) &&
        (distanceMm < gStartupFullSpeedDistanceMm)) {
        int32_t distanceTarget =
            PB22_PB23_STARTUP_MIN_PWM_PERMILLE +
            (int32_t)(((uint64_t)(effectiveTarget -
                PB22_PB23_STARTUP_MIN_PWM_PERMILLE) * distanceMm) /
                gStartupFullSpeedDistanceMm);
        if (distanceTarget < effectiveTarget) {
            effectiveTarget = distanceTarget;
        }
    }

    if (gRampedBasePermille < effectiveTarget) {
        bool initialLaunch =
            gRampedBasePermille < LINE_STEERING_FULL_RAMP_PERMILLE;

        gRampDownIntervalCounter = 0U;
        if (initialLaunch || (gRampUpIntervalTicks <= 1U)) {
            gRampUpIntervalCounter = 0U;
        } else if (++gRampUpIntervalCounter < gRampUpIntervalTicks) {
            return;
        } else {
            gRampUpIntervalCounter = 0U;
        }
        ...
        if (gRampedBasePermille > effectiveTarget) {
            gRampedBasePermille = effectiveTarget;
        }
    } else if (gRampedBasePermille > gTargetBasePermille) {
        gRampUpIntervalCounter = 0U;
        if (++gRampDownIntervalCounter < gRampDownIntervalTicks) {
            return;
        }
        gRampDownIntervalCounter = 0U;
...
static void updateSegmentSpeedLimit(uint32_t distanceMm)
{
    ...
    if (!gCurveSpeedActive) {
        ...
        if (curveMileageWindow &&
            ...
            if (gCurveSpeedEnterTicks >=
                ...
                gSegmentRampIntervalCounter = 0U;
            }
        } else {
            gCurveSpeedEnterTicks = 0U;
        }
    ...
    if (++gSegmentRampIntervalCounter < gSegmentRampIntervalTicks) {
        return;
    }
    gSegmentRampIntervalCounter = 0U;

    if (gRampedSegmentLimitPermille < targetLimit) {
        gRampedSegmentLimitPermille += gSegmentRampUpStep;
        if (gRampedSegmentLimitPermille > targetLimit) {
            gRampedSegmentLimitPermille = targetLimit;
        }
    } else if (gRampedSegmentLimitPermille > targetLimit) {
        gRampedSegmentLimitPermille -= gSegmentRampDownStep;
        if (gRampedSegmentLimitPermille < targetLimit) {
            gRampedSegmentLimitPermille = targetLimit;
        }
    }
...
void StraightControl_init(void)
{
    ...
    gRampUpIntervalTicks = 1U;
    gRampUpIntervalCounter = 0U;
    gRampDownIntervalTicks = 1U;
    gRampDownIntervalCounter = 0U;
    gStartupFullSpeedDistanceMm = 0U;
    gCurveSpeedActive = false;
    gCurveSpeedEnterTicks = 0U;
    ...
    gSegmentRampUpStep = PA18_SEGMENT_RAMP_UP_STEP_PER_10MS;
    gSegmentRampDownStep = PA18_SEGMENT_RAMP_DOWN_STEP_PER_10MS;
    gSegmentRampIntervalTicks = 1U;
    gSegmentRampIntervalCounter = 0U;
...
void StraightControl_start(int32_t targetBasePermille,
    ...
    uint8_t rampUpIntervalTicks,
    uint8_t rampDownIntervalTicks,
    int32_t segmentRampUpStep,
    int32_t segmentRampDownStep,
    uint8_t segmentRampIntervalTicks,
    uint32_t startupFullSpeedDistanceMm)
{
    gActive = true;
    gEncoderFault = false;
    gRampedBasePermille = 0;
    ...
    gRampUpIntervalTicks =
        (rampUpIntervalTicks > 0U) ? rampUpIntervalTicks : 1U;
    gRampUpIntervalCounter = 0U;
    gRampDownIntervalTicks =
        (rampDownIntervalTicks > 0U) ? rampDownIntervalTicks : 1U;
    gRampDownIntervalCounter = 0U;
    gStartupFullSpeedDistanceMm = startupFullSpeedDistanceMm;
    gCurveSpeedActive = false;
    gCurveSpeedEnterTicks = 0U;
    gStraightConfirmTicks = 0U;
    gRampedSegmentLimitPermille = 1000;
    gCurveBasePermille = curveBasePermille;
    gSegmentRampUpStep = segmentRampUpStep;
    gSegmentRampDownStep = segmentRampDownStep;
    gSegmentRampIntervalTicks =
        (segmentRampIntervalTicks > 0U) ? segmentRampIntervalTicks : 1U;
    gSegmentRampIntervalCounter = 0U;
    Encoder_getSnapshot(&gPrevious);
