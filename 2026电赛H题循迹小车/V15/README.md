...
当前按键功能：PA18循迹一圈，忽略起步后第一次启停横线，第二次检测到启停线后立即停止计时并平滑减速停车；PB22执行A→B，进入B端圆弧后继续行驶一个350mm车长，整车通过B点时停止计时，再沿圆弧行驶150mm后减速停车；PB23循迹一圈，整车通过A点时停止计时，再沿直线行驶150mm后减速停车；PB24在任何时刻立即急停。LCD显示陀螺仪Z轴角速度、任务计时、状态、距离和双轮平均实时速度。
...
三种任务的首次上车步骤见`直线测试说明.md`。起步后第一次启停横线会被记录为一圈距离基准，不会触发完成。正常横线接受任意3～8个探头；圆弧出口终点还接受“总共两个探头见黑且至少包含一个最外侧探头”。双灯判据由累计转角达到330°或编码器达到约6.0m后开放，不再依赖会在圆弧末段过期的普通线历史。累计转角达到430°仍未识别横线时进入FAULT停车，避免继续行驶多圈。圆弧控制在连续检测到外侧误差后保留约36‰持续差速，并把弯道基础PWM限制为165‰；反向误差持续50ms即可退出圆弧辅助，避免把弯道差速带入直线。PA18/PB22/PB23都由灰度PID负责方向，编码器用于里程、速度、350mm车长补偿和堵转检测。

V10速度参数：A→B直线巡航上限360‰，绕圈直线巡航上限300‰，弯道仍限制165‰。起步每10ms只增加2‰，到达最大值约需1.5～1.8秒；正常停车每10ms减少6‰，降到0后只施加80‰、40ms的轻反向制动。PA18检测终点后计时立即冻结，但车辆沿轨迹平滑降速；PB24、堵转和超时保护仍立即停机。
