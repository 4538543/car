...
| `stm32_link.c/.h` | UART1非阻塞遥测、角/纵向加速度计算和运动阶段事件通知 |
...
| STM32 通信 | MSP RX ← STM32 TX | PA9 | 55 | UART1_RX | 115200, 8-N-1，当前版本不解析接收数据 |
...
UART1使用`115200, 8-N-1`。接线必须交叉并共地：

- MSPM0 `PB4/UART1_TX` → STM32串口RX；
- MSPM0 `PA9/UART1_RX` ← STM32串口TX；
- MSPM0 GND ↔ STM32 GND。
...
- `WZ`：Z轴角速度，单位`0.01 dps`；
- `ALPHA`：角加速度，单位`0.01 dps²`，由角速度差分并低通滤波得到；
...
发送使用256字节环形队列和UART FIFO轮询，不会阻塞循迹控制。圆弧/直线事件根据陀螺仪角速度滞回识别，仅用于通知STM32，不参与电机PID。
...
三种任务的首次上车步骤见`直线测试说明.md`。起步后第一次启停横线会被记录为一圈距离基准，不会触发完成；第二次横线相对第一次横线判断，因此探头放在启停线后方的距离不会混入一圈长度。横线只要任意3～8个探头见黑即可，不再要求三个探头必须相邻；完成判定要求此前1秒内见过正常1～2灯纵向轨迹。第二次横线判断在相对行程约4.2m后开放，降低编码器误差造成的漏检。PA18/PB22/PB23都由灰度PID负责方向，编码器用于里程、速度、350mm车长补偿和堵转检测。
