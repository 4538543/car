...
static int32_t calculateLinePid(void)
{
    ...
    /*
     * The two center probes form a zero-error acceptance band. Clearing the
     * PID memory here also prevents a derivative kick in the opposite
     * direction when the line returns to center.
     */
    if (error == 0) {
        gLineIntegral = 0;
        gPreviousLineError = 0;
...
void StraightControl_task10ms(void)
{
    ...
    if (StraightControl_hasValidLine()) {
        uint32_t lineErrorMagnitude =
            magnitude32(StraightControl_getLineError());

        if ((lineErrorMagnitude >= LINE_CURVE_SPEED_ERROR_THRESHOLD) &&
            (driveBase > LINE_CURVE_BASE_LIMIT_PERMILLE)) {
            driveBase = LINE_CURVE_BASE_LIMIT_PERMILLE;
        }
    }
