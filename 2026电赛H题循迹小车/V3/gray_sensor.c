...
void GraySensor_sample(void)
{
     ...
     * transverse finish marker covers about four or five adjacent probes.
     * Requiring a central consecutive group rejects a far-edge line, while
     * rejecting seven/eight probes avoids a stuck-low DAT false trigger.
     */
    gSample.finishMarkerCandidate =
        ...
        ((blackMask & GRAY_FINISH_CENTER_SENSOR_MASK) != 0U);
    ...
    /*
     * Treat either of the two center probes as centered for a normal-width
     * line. Without this acceptance band, alternating between the two center
     * probes creates +/-50 error and an unnecessary steering kick.
     */
    if (((blackMask & GRAY_CENTER_ACCEPT_SENSOR_MASK) != 0U) &&
        (activeCount <= GRAY_CENTER_ACCEPT_MAX_ACTIVE_SENSORS)) {
        gSample.lineError = 0;
    } else {
        gSample.lineError =
