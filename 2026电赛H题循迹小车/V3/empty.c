...
static void enableApplicationInterrupts(void)
{
    ...
    const uint32_t keyBMask =
        KEY_GPIO_START_LAP_PIN | KEY_GPIO_EMERGENCY_STOP_PIN;

    DL_GPIO_clearInterruptStatus(
        GPIOA, encoderMask | KEY_GPIO_START_AB_PIN);
...
int main(void)
{
    ...
    while (1) {
        if (takeOneMillisecondTick()) {
            ...
            } else if ((buttonEvents & BUTTON_EVENT_START_AB) != 0U) {
                ABMission_start();
            } else if ((buttonEvents & BUTTON_EVENT_START_LAP) != 0U) {
                ABMission_startLap();
            }
...
void GROUP1_IRQHandler(void)
{
    const uint32_t encoderMask =
        ENCODER_GPIO_E1A_PIN | ENCODER_GPIO_E1B_PIN |
        ENCODER_GPIO_E2A_PIN | ENCODER_GPIO_E2B_PIN;
    const uint32_t gpioAMask = encoderMask | KEY_GPIO_START_AB_PIN;
