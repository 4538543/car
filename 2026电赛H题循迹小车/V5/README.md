...
当前功能：PA18 启动 A→B 任务，到达B附近后通过灰度偏离中心识别圆弧并停车；PB22 启动一圈循迹任务，灰度PID持续通过B/C/D两侧圆弧，回到A附近后识别5cm终点横线，再继续循迹约130～180mm，使终点线进入车身下方后减速制动停车；PB23在任何阶段都可立即停车。两种任务都使用缓慢升速、编码器里程保护、短时反向制动和LCD计时。LCD同时显示陀螺仪Z轴角速度、任务状态、距离和双轮平均实时速度。
...
| `ab_mission.c/.h` | A→B与一圈任务状态机、圆弧处理、终点横线停车和故障保护 |
...
| 按钮 | A→B 启动 | PA18 | 11 | GPIO 输入 | 下拉、按下高、上升沿中断 |
| 按钮 | 行驶一圈 | PB22 | 21 | GPIO 输入 | 上拉、按下低、下降沿中断 |
| 按钮 | 立即停车 | PB23 | 22 | GPIO 输入 | 上拉、按下低、下降沿中断 |
| 按钮 | 备用按钮 | PB24 | 23 | GPIO 输入 | 上拉，暂不开中断 |
...
| `TIME` | 从PA18或PB22确认启动到正常停车、PB23急停或故障停车的时间；停车后数值冻结 |
| `STATE` | `IDLE / CRUISE / SLOW / DECEL / COAST / BRAKE / DONE / ABORT / FAULT` |
| `DIST` | 双轮编码器平均里程，单位mm |
...
直线和一圈功能的首次上车步骤见 `直线测试说明.md`。当前版本已经按照感为厂家串行例程接入灰度辅助板：CLK上升沿后等待约5us，下降沿读取DAT，连续读取8位；默认白色为1、黑色为0。一圈运行时LCD状态显示`LAP RUN`，接近A点减速时显示`LAP SLOW`，确认终点横线后显示`LINE HIT`。弯道中检测线偏离中心时会自动降低平均速度、增强差速修正；短暂丢线时继续按最后方向找线。PA18和PB22现在都由灰度PID负责方向，编码器不再进行累计等距离纠偏，只用于里程、速度和堵转检测。起步PWM低于100‰时，任一中间探头见黑都暂时视为居中；正常行驶后，单个中间探头恢复柔和纠偏，逐渐让两个中间探头共同对准黑线。
