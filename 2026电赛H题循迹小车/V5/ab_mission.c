...
void ABMission_init(void)
{
    ...
    gFinishMarkerConfirmTicks = 0U;
    gFinishMarkerDetected = false;
    gFinishMarkerCount = 0U;
}
    ...
    ABMissionMode mode, int32_t cruisePermille)
{
    ...
    /*
     * Real-vehicle evidence showed that the cumulative equal-distance loop
     * makes PA18 oscillate, while PB22 is smooth with it disabled. In both
     * modes the grayscale PID now owns steering; encoders remain active for
     * distance, speed display and stalled-wheel protection.
     */
...
void ABMission_task1ms(void)
{
    ...
    if (gMode == AB_MISSION_MODE_LAP) {
        if (!gFinishMarkerDetected) {
            ...
            if (averageCount >= LAP_FINISH_ARM_COUNT) {
                if (GraySensor_isFinishMarkerCandidate()) {
                    if (gFinishMarkerConfirmTicks <
                        LAP_FINISH_CONFIRM_10MS_TICKS) {
                        gFinishMarkerConfirmTicks++;
                    }
                } else {
                    gFinishMarkerConfirmTicks = 0U;
                }
                ...
                if (gFinishMarkerConfirmTicks >=
                    ...
                    gFinishMarkerCount = averageCount;
                    StraightControl_setTargetBase(
                        LAP_FINISH_COVER_PWM_PERMILLE);
                    gState = AB_MISSION_FINISH_COVER;
                    return;
                }
            }

            /*
             * A missed finish marker must not leave the vehicle circling
             * indefinitely. This is a safety fault, not normal completion.
             */
            if (averageCount >= LAP_MISSED_MARKER_STOP_COUNT) {
                finishMission(AB_MISSION_FAULT);
            }
            return;
        } else {
            uint32_t countAfterMarker =
                (averageCount >= gFinishMarkerCount) ?
                (averageCount - gFinishMarkerCount) : 0U;

            if (countAfterMarker >= LAP_FINISH_STOP_AFTER_COUNT) {
                beginBrakeWait();
                return;
            }
