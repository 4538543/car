...
当前功能：PA18 启动 A→B 任务，到达B附近后通过灰度偏离中心识别圆弧并停车；PB22 启动一圈循迹任务，灰度PID持续通过B/C/D两侧圆弧，接近理论一圈6.142m时减速并回到A附近停车；PB23在任何阶段都可立即停车。两种任务都使用缓慢升速、编码器里程保护、短时反向制动和LCD计时。LCD同时显示陀螺仪Z轴角速度、任务状态和距离。
...
| `ab_mission.c/.h` | A→B与一圈任务状态机、圆弧处理、里程停车和故障保护 |
...
直线和一圈功能的首次上车步骤见 `直线测试说明.md`。当前版本已经按照感为厂家串行例程接入灰度辅助板：CLK上升沿后等待约5us，下降沿读取DAT，连续读取8位；默认白色为1、黑色为0。一圈运行时LCD状态显示`LAP RUN`，接近A点减速时显示`LAP SLOW`。
