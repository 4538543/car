...
void GraySensor_init(void)
{
    ...
    gSample.activeCount = 0U;
    gSample.lineError = 0;
    gSample.lineValid = false;
    gSample.finishMarkerCandidate = false;
}
...
void GraySensor_sample(void)
{
    ...
    uint8_t consecutiveCount = 0U;
    uint8_t longestConsecutiveCount = 0U;
    uint8_t index;
    ...
    for (index = 0U; index < 8U; index++) {
        ...
        if (level == GRAY_BLACK_LEVEL) {
            ...
            activeCount++;
            consecutiveCount++;
            if (consecutiveCount > longestConsecutiveCount) {
    ...
    gSample.activeCount = activeCount;
    /*
     * A normal 18 mm longitudinal line lights one or two probes. The 50 mm
     * transverse start/finish marker can cover three through all eight
     * probes. Individual threshold differences can make the active probes
     * non-contiguous, so three or more active probes are sufficient. The
     * mission layer still requires a recent normal 1/2-probe line before
     * accepting the lap-ending marker; continuously stuck-low DAT is not
     * accepted as a completion marker.
     */
    gSample.finishMarkerCandidate =
        (activeCount >= GRAY_FINISH_MIN_ACTIVE_SENSORS) &&
        (activeCount <= GRAY_FINISH_MAX_ACTIVE_SENSORS) &&
        (longestConsecutiveCount >=
            GRAY_FINISH_MIN_CONSECUTIVE_SENSORS);
    ...
    /*
     * Preserve the real +/-50 error when only one center probe sees black.
     * StraightControl suppresses this tiny error only during startup; once
     * moving, it is needed to remove a persistent half-probe lateral offset.
     */
    gSample.lineError =
        (int16_t)(weightedSum / (int32_t)activeCount);
...
{
    return gSample.activeCount;
}

bool GraySensor_isFinishMarkerCandidate(void)
{
    return gSample.finishMarkerCandidate;
}
