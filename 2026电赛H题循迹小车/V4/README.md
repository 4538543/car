...
直线和一圈功能的首次上车步骤见 `直线测试说明.md`。当前版本已经按照感为厂家串行例程接入灰度辅助板：CLK上升沿后等待约5us，下降沿读取DAT，连续读取8位；默认白色为1、黑色为0。一圈运行时LCD状态显示`LAP RUN`，接近A点减速时显示`LAP SLOW`，确认终点横线后显示`LINE HIT`。弯道中检测线偏离中心时会自动降低平均速度、增强差速修正；短暂丢线时继续按最后方向找线。PA18和PB22现在都由灰度PID负责方向，编码器不再进行累计等距离纠偏，只用于里程、速度和堵转检测。
