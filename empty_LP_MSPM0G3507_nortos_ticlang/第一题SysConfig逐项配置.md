...
| 左编码器 A 相 | E1A | PB25 | GPIO 输入、中断 | Rising Edge |
| 左编码器 B 相 | E1B | PA23 | GPIO 输入 | 无中断 |
...
|---|---|---|---|---|---|
| ENCODER_E1A | Input | No Resistor | Enabled | Rising Edge | 25 / PB25 |
| ENCODER_E2A | Input | No Resistor | Enabled | Rising Edge | 21 / PB21 |
| ENCODER_E2B | Input | No Resistor | Disabled | 不适用 | 27 / PB27 |

左编码器 B 相单独再加一个 GPIO 实例：

- `Name`：ENCODER_E1B_GPIO
- `Port`：PORTA
- `Port Segment`：Upper
- `Pin Name`：ENCODER_E1B
- `Direction`：Input
- `Internal Resistor`：No Resistor
- `GPIO Interrupt`：Disabled
- `Assigned Pin / PinMux`：23 / PA23

只对两路 A 相开上升沿中断。当前代码只用 A 相计数，B 相作为保留输入；不要同时打开下降沿，否则每圈计数倍数会改变，速度 PID 参数也必须重标。
...
4. PB25/PB21 只开 Rising Edge；PA23/PB27 不开中断。
